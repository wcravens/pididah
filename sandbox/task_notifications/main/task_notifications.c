...
static void gpio_task_handler(void* pvParameters) {
    ...
    for(;;) {
      ...
      if( xEvent == pdPASS ) {
        ...
        if( now - lastTick > 5 ) {
          lastTick = now;
          pinState = gpio_get_level( pinNum );
...
void configure_event_management() {
  ...
  gpio_isr_handler_add( GPIO_LEFT_PADDLE,   gpio_isr_handler, (void*) &left_paddle_task_handle  );
  gpio_isr_handler_add( GPIO_RIGHT_PADDLE,  gpio_isr_handler, (void*) &right_paddle_task_handle );
}
